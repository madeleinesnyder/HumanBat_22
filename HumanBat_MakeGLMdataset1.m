% Make dataset #1 
% Run GLM to predict neural firing at any given timestep for lag = 0 using
    % K position
    % M positon
    % B position
    % B_other position
...
%% Make training dataset (1) (All flights, -3s and +3s)
for i=1:length(ciholas_flight_struct_chron)

    temp_fstart = ciholas_flight_struct_chron{i}.fstart_trimmed-seconds*120;
    temp_fend = ciholas_flight_struct_chron{i}.fend_trimmed+seconds*120;
    ...
    for j=1:length(M_distance_from_tripods)
        dataset1.M_tripod{i} = find(M_distance_from_tripods == min(M_distance_from_tripods(j)));
    end
    ...
    for j=1:length(K_distance_from_tripods)
        dataset1.K_tripod{i} = find(K_distance_from_tripods == min(K_distance_from_tripods(j)));
    end
    %figure(); hold on; xlim([-2900 2900]); ylim([-2600 2600]);
    ...
    if isempty(ephys_idxs_in_range)
        dataset1.E{i} = 0; % zeros(1,length(tt));
        dataset1.E_samples{i} = length(tt);
    else
    ...
    dataset1.B_other{i} = temp_bin_rollout;     
end

%% Table 1:

E_cat = []; K_cat = []; M_cat = []; B_cat = []; B_other_cat = [];
for i=1:20%length(dataset1.B)

    % Concatenate the ephys data
    E_zeros = zeros(1,dataset1.E_samples{i});
    try
        E_zeros(dataset1.E{i}) = 1;
    catch
        disp("No spikes for this flight");
    end
    to_trim = mod(length(E_zeros),length(dataset1.B{i}));
    E_zeros = E_zeros(to_trim+1:end)';
    E_cat = [E_cat; E_zeros];

    % Concatenate and resample the position data (LAGGING DUE TO ROUNDING)
    B_resampled = [];
    resample_factor = length(E_zeros)/length(dataset1.B{i});
    for j=1:length(dataset1.B{i})
        B_resampled = [B_resampled;repmat(dataset1.B{i}(j),1,resample_factor)'];
    end
    B_cat = [B_cat;B_resampled];

    B_other_resampled = [];
    resample_factor_o = length(E_zeros)/length(dataset1.B_other{i});
    for j=1:length(dataset1.B_other{i})
        B_other_resampled = [B_other_resampled;repmat(dataset1.B{i}(j),1,resample_factor_o)'];
    end
    B_other_cat = [B_cat;B_other_resampled];

    % Concatenate and resample the M and K data 
    K_resampled = []; M_resampled = [];
    resample_factor_KM = length(E_zeros)/length(dataset1.M_tripod{i});
    M_resampled = [M_resampled;repmat(dataset1.M_tripod{i},1,resample_factor_KM)'];
    K_resampled = [K_resampled;repmat(dataset1.K_tripod{i},1,resample_factor_KM)'];
...
% Make a table
table1 = [K_cat,B_cat,M_cat];
K_cat=[]; B_cat=[];M_cat=[]; K_resampled=[]; M_resampled=[]; B_other_resampled=[];
