function HumanBat_extractGlobalAudio(audio_clip,audio_loaded,ttl_clip,ttl_loaded,varargin)
...
% Load mic data and ttl data
mic_file_name = audio_clip;
ttl_file_name = ttl_clip;
disp(mic_file_name)
disp(ttl_file_name);

if isempty(mic_file_name)
    file_ = audio_loaded;
else
    load(audio_clip);
end

if isempty(ttl_file_name)
    file_ = ttl_loaded;
else
    load(ttl_clip);
end
...
% Load in flight data 
wd = pwd; file_date = wd(end-24:end-19);
load(strcat('../../../../../processed/',file_date,'/b149f/cortex/',file_date,'_14592_tracking_1_track.mat'));
...
% Convert audio signal to global time
...
figure();   set(gcf, 'units','normalized','outerposition',[0.5 0 0.5 1]);   %x_axis
for i=1:n_tags
    ax_0(i) = subplot(n_tags+1,1,i);
    plot((tag_data{1, i}(:,2)-t0)*ntw_time/60, tag_data{1, i}(:,3));              sgtitle('x');
end
ax_0(n_tags+1) = subplot(n_tags+1,1,n_tags+1);
plot((sync_data(:,2)-t0)*ntw_time/60, sync_data(:,3));              sgtitle('x');
linkaxes(ax_0,'x');    xlabel('Time(s)');

figure();   set(gcf, 'units','normalized','outerposition',[0.5 0 0.5 1]);   %x_axis
for i=1:n_tags
    ax_a(i) = subplot(n_tags+1,1,i);
    plot((tag_ac_data{1, i}(:,2)-t0)*ntw_time/60, tag_ac_data{1, i}(:,3));              sgtitle('x');
end
ax_a(n_tags+1) = subplot(n_tags+1,1,n_tags+1);
plot((sync_data(:,2)-t0)*ntw_time/60, sync_data(:,3));              sgtitle('x');
linkaxes(ax_a,'x');    xlabel('Time(s)');
...
if use_sync
    ...
    subplot(212);   findpeaks(normalize(sync_data(:,3),'zscore'),sync_data(:,2),'MinPeakHeight',1,'MinPeakDistance',2/ntw_time);   xlabel('Packet#');     ylabel('Sync signal (a.u.)');
    
    %If no errors in TTL detection -> interpolate time from known TTL delays 
    if ~sync_err
...
if ~isempty(find((RTLS_data(:,1)==0 & RTLS_data(:,2)==tags_SN(1))))
    save([parent_path, '/extracted_', fn, '.mat'],'tag_data','sync_data','CDPmtdata','tag_data_filt','tag_ac_data', 'TTL_network_times');
else
    save([parent_path, '/extracted_', fn, '.mat'],'tag_data','sync_data','CDPmtdata','tag_data_filt');
end
...
figure();   set(gcf, 'units','normalized','outerposition',[0.5 0 0.5 1]);   %y_axis
for i=1:n_tags
    ax(i) = subplot(n_tags,1,i);
...
figure();   set(gcf, 'units','normalized','outerposition',[0.5 0 0.5 1]);   %z_axis
for i=1:n_tags
    ax(i) = subplot(n_tags,1,i);
...
%Sync
figure();   set(gcf, 'units','normalized','outerposition',[0.5 0 0.5 1]);   sgtitle('Sync data');
...
%Tags
figure();   set(gcf, 'units','normalized','outerposition',[0.5 0 0.5 1]);   sgtitle('Tags data');
for i=1:n_tags
