function Process_Collective_Behavior_AF_v0()
%% Function for the pre-processing of collective behavior (March 2022)
% Requires extracted data from Ciholas recordings and Group name (D or F)

disp('...Processing Ciholas Data...');

%=== Load data and group name
extracted_CDPfile = dir(fullfile(cd, '*extracted_*'));          load(extracted_CDPfile.name);     %load('Group_name.mat');
...
bat_nms = ['KQ_RH';'MS_RH';'KQ_RP';'MS_RP';'MS_LH';'KQ_LH';'BLOND'];
...
% t should be between 0 and the length of the cortex tt recording (cover
% the whole cortex data stream)
t = [ciholas_data.CDPmtdata.TTL_times(1):1/Fs:ciholas_data.CDPmtdata.TTL_times(end)]';       %Evenly sampled time vector from first to last TTL.
...
for i = 1:n_tags
    r(:,:,i) =  interp1(ciholas_data.tag_data_filt{1,7}(:,8), ciholas_data.tag_data_filt{1,7}(:,[3:5]),t,'nearest','extrap'); %DO NOT use spline interpolation!
end
...
if exist('tag_ac_data')
    a = zeros(T,3,n_tags);
    for i = 1:n_tags
        a(:,:,i) =  interp1(ciholas_data.tag_ac_data{1,7}(:,8), ciholas_data.tag_ac_data{1,7}(:,[3:5]),t,'nearest','extrap'); %DO NOT use spline interpolation!
    end
...
if exist('tag_ac_data')
    for i = 1:n_tags
        ...
        wBeats(:,i) = movsum(env>env_th,2*Fs)>Fs/5; 
        figure(); hold on; %Euristic criterion for flight detection
                         ax(i) = subplot(n_tags,1,i);  area(t,wBeats(:,i)*3,'FaceAlpha',0.3,'LineStyle','none');  hold on;
...
if exist('tag_ac_data')
    ...
    % % Uncomment to control filtering quality
     ax(1) = subplot(311);  plot(tag_data{1,7}(:,8),tag_data{1,7}(:,3)',t,r_stat(:,1,7));
     ax(2) = subplot(312);  plot(tag_data{1,7}(:,8),tag_data{1,7}(:,4)',t,r_stat(:,2,7));
     ax(3) = subplot(313);  plot(tag_data{1,7}(:,8),tag_data{1,7}(:,5)',t,r_stat(:,3,7));
     linkaxes(ax,'x');
...
if options.show_fig
    ...
    for j = 1:3
        ...
        tiledlayout(n_tags,1,'TileSpacing','tight');
            ax(i) = nexttile; 
            plot(ciholas_data.tag_data{1, 7}(:,8), ciholas_data.tag_data{1, 7}(:,2+j),t,r_corr(:,j));
