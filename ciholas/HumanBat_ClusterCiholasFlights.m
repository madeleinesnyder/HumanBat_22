function [ciholas_flights] = HumanBat_ClusterCiholasFlights(exp_data_path,batdate)
    % Based on Angelo's Analyse_Collective_Behavior script

    %=== Load data from HumanBat_SegmentCiholasTrajectories
    load(strcat(exp_data_path,'ciholas/Extracted_Behavior_',num2str(batdate),'.mat'));

    %=== Parameters
    n_ds = Fs;                                                                                                     % Downsampling factor for clustering positions
    n_min = 8.4*Fs;                                                                                                  % Minimum number of samples to be considered a cluster (at least 10s)
...
%     end
    ...
    if f_num>0
        v_diff = diff(v_abs>v_th);
        transitions = find(v_diff);
        if v_diff(transitions(1))   ==-1,b_corr(1:transitions(1)) = 0;    end
        if v_diff(transitions(end)) == 1,b_corr(transitions(end):end) = 0;end
    end
...
%     
    ...
    X = [];  X = [X;r(~bflying& b_corr,:)]; 
    figure();   set(gcf, 'units','normalized','outerposition',[0.3 0.2 0.2 0.2]);
    [~,centroid_all] = Cluster3D_AF_v1(X,n_min,0.2,Fs);
...
%     end
    ...
    X = r(~bflying& b_corr,:);                                                              % Consider only stationary epochs
    [id,centroid{1},samplesIn{1},p_val_clus,CH_idx] = Cluster3D_AF_v1(X,n_min,0.2,n_ds);  % Perform clustering
    perc_clus = nnz(id)/length(id);                                                                % fraction of points belonging to a cluster
    r_clus_id(~bflying& b_corr)=id;                                                         % Assign cluster id across entire session (nan when bat is flying)
    %fig_count = saveFig(analysis_directory,batdate,fig_count,options.savefigures);
    ...
    elseif batdate==220414
        ...
        alpha_clus = 1.6;         %Parameter for flight clustering
    end
    f_cls = cell(n_tags,1);     
    f_clus = FlightClus_AF_v2(squeeze(r(:,:)),bflying,'Alpha',alpha_clus,'Frechet',0,'Points',10);
    f_cls{1} = f_clus.sorted_ids;
    %fig_count = saveFig(analysis_directory,batdate,fig_count,options.savefigures);
    f_clus = struct([]);
    %f_cls{1} = 0.*f_smp{1};  
    ciholas_flights.cluster_idx = f_cls{1};
    ...
    c = 1;  FLIGHTS = table();
    for j=1:f_num
        FLIGHTS.id(c,:) = 1;                                                        % id of the bat flying
        FLIGHTS.smp1(c,:) = f_smp{1}(j,1);                                        % sample takeoff
        FLIGHTS.smp2(c,:) = f_smp{2}(j,1);                                        % sample landing
        FLIGHTS.t1(c,:) = t(f_smp{1}(j,1));                                       % time takeoff
        FLIGHTS.t2(c,:) = t(f_smp{2}(j,1));                                       % time landing
        FLIGHTS.maxN(c,:) = max(sum(bflying(f_smp{1}(j,1):f_smp{2}(j,1),:),2)); % max number of bats flying during that flight
        FLIGHTS.r1(c,:) = r(f_smp{1}(j,1),:);                                   % position at takeoff
        FLIGHTS.r2(c,:) = r(f_smp{2}(j,1),:);                                   % position at landing
        FLIGHTS.pclus1(c,:) = r_clus_id(f_smp{1}(j,1)-1);                       % positional cluster at take off
        FLIGHTS.pclus2(c,:) = r_clus_id(f_smp{2}(j,1)+1);                       % positional cluster at landing
        FLIGHTS.fclus(c,:) = f_cls{1}(1,j);                                       % flight cluster
...
%             end
        ...
        if ~isempty(min(t(f_smp{1}(j,1))-all_lnd(all_lnd<t(f_smp{1}(j,1)))))
            FLIGHTS.t_prev_lnd(c,:) = min(t(f_smp{1}(j,1))-all_lnd(all_lnd<t(f_smp{1}(j,1))));
        else
            FLIGHTS.t_prev_lnd(c,:) = Inf;
        end
        if ~isempty(min(-t(f_smp{2}(j,1))+all_tko(all_tko>t(f_smp{2}(j,1)))))
            FLIGHTS.t_next_tko(c,:) = min(-t(f_smp{2}(j,1))+all_tko(all_tko>t(f_smp{2}(j,1))));
        else
    ...
    warning('on','all');
    ciholas_flights.flights = FLIGHTS;
...
%     fig_count = saveFig(analysis_directory,batdate,fig_count,options.savefigures);
    ...
    extended_flight = movmax(bflying,[1*Fs 1*Fs]);
    trace = a_abs(find(extended_flight));
    ...
    hold on;
    plot(f,sm_PSD,'LineWidth',4,'Color',bat_clr);
    for j = 1:5,plot(wb_F*[j j],ylim,'--k');end
...
%         bat.f_num = f_num;
%         bat.p_val_clus = p_val_clus;
%         bat.CH_idx = CH_idx;
%         bat.perc_clus = perc_clus;
%         bat.p_fly1 = p_fly1;
%         bat.wb_F = wb_F;
        
        %=== Exploration ratio see Obenhaus et al., "Functional network topography of the medial entorhinal cortex." PNAS 2022)
%         occupancy = histcounts2(r(~bflying& b_corr,1),r(~bflying& b_corr,2),cell2mat(edges_d(1,1)),cell2mat(edges_d(1,2)));
%         perimeter = [occupancy(1,:),occupancy(end,:),occupancy(:,1)',occupancy(:,end)'];
%         bat.exp_ratio_all = nnz(occupancy>Fs*5)/numel(occupancy);
%         bat.exp_ratio_per = nnz(perimeter>Fs*5)/numel(perimeter);
%         bat.spatial_map = occupancy./T;
    
        %=== Fraction of time spent close to the feeder
%         for j = 1:size(r_fd,1),condition(:,j) = vecnorm(r(:,:)-r_fd(j,:),2,2)<d_th;end
%         bat.feedfraction = nnz(any(condition,2))/T;
%         
