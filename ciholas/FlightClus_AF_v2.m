function flight_clus = FlightClus_AF_v2(r_vector,f_vector,varargin)
...
x2=2.9; x1=-2.9; y2=2.6;  y1=-2.6;  z1=0; z2=2.30;  %flight volume coordinates
...
%=== Plot distances between flights and dendrogram
figure('units','normalized','outerposition',[0.5 0.2 0.15 0.7]);
subplot(311); edges_f = 10.^linspace(log10(min(Y)),log10(max(Y)),50);  histogram(Y,edges_f);    set(gca,'XScale','log');    set(gca,'YScale','log'); xlabel('Inter-flight distances (m)');
subplot(312); histogram(Y); xlabel('Inter-flight distances (m)');
subplot(313); dendrogram(Z,0);  hold on;    refline(0,dist);    hold off;
...
for jj=1:n_surv_clusters
    ...
    plot3(r_vector(:,1),r_vector(:,2),r_vector(:,3),':','Color',[0.8 0.8 0.8],'MarkerSize',0.001);
    xlim([x1 x2]); ylim([y1 y2]);   zlim([z1 z2]);  view(0,90); xlabel('x');    ylabel('y');    hold on;
    avg_take_off = [];
    for ii=1:size(id,2)
        title(['Cluster' num2str(jj) ' (' num2str(size(id,2)) ' flights),' num2str(size(id,2)/num_flights*100,2) '%'])
        plot3(flight_clus.pos(1,:,id(ii)),flight_clus.pos(2,:,id(ii)),flight_clus.pos(3,:,id(ii)),'-','LineWidth',1.5,'Color', col(jj,:));
    ...
    take_off = mean(avg_take_off,2);   hold off; %textscatter(take_off(1),take_off(2),"Take-off");        hold off;

    nexttile(1*n_surv_clusters+jj);
    plot(r_vector(:,1),r_vector(:,3),':','Color',[0.8 0.8 0.8],'MarkerSize',0.001); 
    xlim([x1 x2]); ylim([z1 z2]);       xlabel('x');    ylabel('y');    hold on;
    avg_take_off = [];
    for ii=1:size(id,2)
    ...
    take_off = mean(avg_take_off,2);   hold off; %textscatter(take_off(1),take_off(3),"Take-off");     hold off;
    
    nexttile(2*n_surv_clusters+jj);
    histogram(flight_clus.dur(id)); xlim([0 15]);   xlabel('Duration(s)');  ylabel('Counts');
end
