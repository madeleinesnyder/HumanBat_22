function [sig_pre,sig_land] = HumanBat_MvK_Permutation_Clusters_Cortex(clus,unit,Flight_Group_Matrix,cortex_flight_struct_resort,B_ephys_data)
...
for clust = clus
    ...
    if length(Mx_flights) < 3 | length(Kx_flights) < 3
        ...
        for uu=unit %1:length(cortex_flight_struct_resort{1}.ephys_trimmed)
            mean_spike_rate_pre = [];
            unit = uu;
            ...
            for n=1:500
                ...
                for i=1:length(random_indexes_subset1)
                    pre_flight_start = cortex_flight_struct_resort{All_clus_flights(random_indexes_subset1(i))}.fstart_trimmed-sec*120;
                    pre_flight_end = cortex_flight_struct_resort{All_clus_flights(random_indexes_subset1(i))}.fstart_trimmed;
                ...
                for i=1:length(random_indexes_subset2)
                    pre_flight_start = cortex_flight_struct_resort{All_clus_flights(random_indexes_subset2(i))}.fstart_trimmed-sec*120;
                    pre_flight_end = cortex_flight_struct_resort{All_clus_flights(random_indexes_subset2(i))}.fstart_trimmed;
                    flight_ephys_B = {}; ephys=0;   tt= {}; rr={};
                    f_start_seconds = pre_flight_start/cortex_fs;  f_end_seconds = pre_flight_end/cortex_fs;   % Get ephys samples in seconds
                    % Find the closest ephys sample to f_start_seconds*1e6 
                    ephys_f_start = f_start_seconds*1e6; % This is the start time for that ephys chunk. Cut out that ephys chunk
            ...
            for i=1:length(Kx_flights)
                pre_flight_start = cortex_flight_struct_resort{Kx_flights(i)}.fstart_trimmed-sec*120;
                pre_flight_end = cortex_flight_struct_resort{Kx_flights(i)}.fstart_trimmed;
                flight_ephys_B = {}; ephys=0;   tt= {}; rr={};
                f_start_seconds = pre_flight_start/cortex_fs;  f_end_seconds = pre_flight_end/cortex_fs;   % Get ephys samples in seconds
                % Find the closest ephys sample to f_start_seconds*1e6 
                ephys_f_start = f_start_seconds*1e6; % This is the start time for that ephys chunk. Cut out that ephys chunk
            ...
            for i=1:length(Mx_flights)
                pre_flight_start = cortex_flight_struct_resort{Mx_flights(i)}.fstart_trimmed-sec*120;
                pre_flight_end = cortex_flight_struct_resort{Mx_flights(i)}.fstart_trimmed;
                flight_ephys_B = {}; ephys=0;   tt= {}; rr={};
                f_start_seconds = pre_flight_start/cortex_fs;  f_end_seconds = pre_flight_end/cortex_fs;   % Get ephys samples in seconds
                % Find the closest ephys sample to f_start_seconds*1e6 
                ephys_f_start = f_start_seconds*1e6; % This is the start time for that ephys chunk. Cut out that ephys chunk
...
%             clear numSpikes;
%             for i=1:length(Kx_flights)
%                 pre_flight_start = cortex_flight_struct_resort{Kx_flights(i)}.fend_trimmed;
%                 pre_flight_end = cortex_flight_struct_resort{Kx_flights(i)}.fend_trimmed+sec*120;
%                 flight_ephys_B = {}; ephys=0;   tt= {}; rr={};
%                 f_start_seconds = pre_flight_start/cortex_fs;  f_end_seconds = pre_flight_end/cortex_fs;   % Get ephys samples in seconds
%                 % Find the closest ephys sample to f_start_seconds*1e6 
%                 ephys_f_start = f_start_seconds*1e6; % This is the start time for that ephys chunk. Cut out that ephys chunk
...
%             clear numSpikes;
%             for i=1:length(Mx_flights)
%                 pre_flight_start = cortex_flight_struct_resort{Mx_flights(i)}.fend_trimmed;
%                 pre_flight_end = cortex_flight_struct_resort{Mx_flights(i)}.fend_trimmed+sec*120;
%                 flight_ephys_B = {}; ephys=0;   tt= {}; rr={};
%                 f_start_seconds = pre_flight_start/cortex_fs;  f_end_seconds = pre_flight_end/cortex_fs;   % Get ephys samples in seconds
%                 % Find the closest ephys sample to f_start_seconds*1e6 
%                 ephys_f_start = f_start_seconds*1e6; % This is the start time for that ephys chunk. Cut out that ephys chunk
...
%     
        ...
        for uu=unit %1:length(cortex_flight_struct_resort{1}.ephys_trimmed)
            ...
            for n=1:500
                ...
                random_indexes_subset2 = find(CFV==1);
                for i=1:length(random_indexes_subset1)
                    pre_flight_start = cortex_flight_struct_resort{All_clus_flights(random_indexes_subset1(i))}.fend_trimmed-0.25*120;
                    pre_flight_end = cortex_flight_struct_resort{All_clus_flights(random_indexes_subset1(i))}.fend_trimmed+60;
                    flight_ephys_B = {}; ephys=0;   tt= {}; rr={};
                    f_start_seconds = pre_flight_start/cortex_fs;  f_end_seconds = pre_flight_end/cortex_fs;   % Get ephys samples in seconds
                    % Find the closest ephys sample to f_start_seconds*1e6 
                    ephys_f_start = f_start_seconds*1e6; % This is the start time for that ephys chunk. Cut out that ephys chunk
                ...
                % Now for subset2
                for i=1:length(random_indexes_subset2)
                    pre_flight_start = cortex_flight_struct_resort{All_clus_flights(random_indexes_subset2(i))}.fend_trimmed-0.25*120;
                    pre_flight_end = cortex_flight_struct_resort{All_clus_flights(random_indexes_subset2(i))}.fend_trimmed+60;
                    flight_ephys_B = {}; ephys=0;   tt= {}; rr={};
                    f_start_seconds = pre_flight_start/cortex_fs;  f_end_seconds = pre_flight_end/cortex_fs;   % Get ephys samples in seconds
                    % Find the closest ephys sample to f_start_seconds*1e6 
                    ephys_f_start = f_start_seconds*1e6; % This is the start time for that ephys chunk. Cut out that ephys chunk
            ...
            clear numSpikes;
            for i=1:length(Kx_flights)
                pre_flight_start = cortex_flight_struct_resort{Kx_flights(i)}.fend_trimmed-0.25*120;
                pre_flight_end = cortex_flight_struct_resort{Kx_flights(i)}.fend_trimmed+60;
                flight_ephys_B = {}; ephys=0;   tt= {}; rr={};
                f_start_seconds = pre_flight_start/cortex_fs;  f_end_seconds = pre_flight_end/cortex_fs;   % Get ephys samples in seconds
                % Find the closest ephys sample to f_start_seconds*1e6 
                ephys_f_start = f_start_seconds*1e6; % This is the start time for that ephys chunk. Cut out that ephys chunk
            ...
            clear numSpikes;
            for i=1:length(Mx_flights)
                pre_flight_start = cortex_flight_struct_resort{Mx_flights(i)}.fend_trimmed-0.25*120;
                pre_flight_end = cortex_flight_struct_resort{Mx_flights(i)}.fend_trimmed+60;
                flight_ephys_B = {}; ephys=0;   tt= {}; rr={};
                f_start_seconds = pre_flight_start/cortex_fs;  f_end_seconds = pre_flight_end/cortex_fs;   % Get ephys samples in seconds
                % Find the closest ephys sample to f_start_seconds*1e6 
                ephys_f_start = f_start_seconds*1e6; % This is the start time for that ephys chunk. Cut out that ephys chunk
...
%                 random_indexes_subset2 = find(CFV==1);
%                 for i=1:length(random_indexes_subset1)
%                     pre_flight_start = cortex_flight_struct_resort{All_clus_flights(random_indexes_subset1(i))}.fstart_trimmed;
%                     pre_flight_end = cortex_flight_struct_resort{All_clus_flights(random_indexes_subset1(i))}.fend_trimmed;
%                     flight_ephys_B = {}; ephys=0;   tt= {}; rr={};
%                     f_start_seconds = pre_flight_start/cortex_fs;  f_end_seconds = pre_flight_end/cortex_fs;   % Get ephys samples in seconds
%                     % Find the closest ephys sample to f_start_seconds*1e6 
%                     ephys_f_start = f_start_seconds*1e6; % This is the start time for that ephys chunk. Cut out that ephys chunk
