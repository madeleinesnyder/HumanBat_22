...
ind=findstr(FullFileName,filesep);
...
while Ncharacters > 0 % The end of the parameter record is indicated by <0 characters for group/parameter name
    ...
    if GroupNumber<0 % Group data
        ...
        elseif type==2 & datalength>0			%integer
            ...
            Nparameters=datalength/abs(type);	
            %%%%THIS WAS CHANGED FROM AN INT16 TO A UINT16 SOLELY FOR THE
            %%%%PURPOSE OF GETTING THE ACTUAL START FIELD AND ACTUAL END FIELD PARAMETERS, THIS
            %%%%MIGHT NOT WORK FOR OTHER PARAMETERS
...
%%%GET ACUTAL_START_FIELD AND ACTUAL_END_FIELD (it might be better to
%%%actually find this data, rather than assume that it is always in a
%%%specific place

if size(ParameterGroup,2) > 4 %this is necessary for the room marker data 
    actual_start_field = ParameterGroup(5).Parameter(1).data; %this should be two numbers
    actual_start_word1 = actual_start_field(1);
    actual_start_word2 = actual_start_field(2);
    StartFrame = typecast([uint16(actual_start_word1) uint16(actual_start_word2)],'uint32');
    
    actual_end_field = ParameterGroup(5).Parameter(2).data; %this should be two numbers
    actual_end_word1 = actual_end_field(1);
    actual_end_word2 = actual_end_field(2);
    
    EndFrame = typecast([uint16(actual_end_word1) uint16(actual_end_word2)],'uint32');
end
...
if Scale < 0
    for i=1:NvideoFrames
        if(mod(i,10000) == 0)
            disp(sprintf('Extracted %d Cortex Video Frames!',i));
        end
        for j=1:Nmarkers
...
%             end
            ...
            %commenting out CameraInfo and ResidualError to save memory
...
%             end
    ...
    %commenting this out in order to make sure of what it is doing 
% % %     for i=1:NvideoFrames
% % %         for j=1:Nmarkers
