function [PSTH_mn,PSTH_cs,PSTH_up,PSTH_dn] = Raster_AF_v6_cortex(s,pts1,pts2,ends1,ends2,ids1,ids2,clrs1,clrs2,interval,cv1,cv2,ptitle1,ptitle2,PSTH_flag,fig_flag,Mx_flights,Kx_flights,cortex_flight_struct_resort,clus)
...
list1=[]; list2=[];
[pts_all,pia] = sort([pts1;pts2]); ids_all = ([ids1;ids2]); ends_all = sort([ends1,ends2]');
for i=1:length(pts1)
...
if fig_flag
    ...
    for j = 1:numel(group_id)
        ...
        if PSTH_flag
            plot(PSTH_cs,(A_factor1/max(PSTH_mn1(:)))*PSTH_mn1(j,:)-last_event_all(j),'Color', [g_clr1(j,:) 0.7],'LineWidth',2);
        end
    end
    ...
    K_cols = [1 0 0; 1 0.2 0.2; 1 0.3 0.3; 1 0.4 0.4; 1 0.5 0.5; 1 0.55 0.55; 1 0.6 0.6; 1 0.65 0.65; 1 0.7 0.7; 1 0.75 0.75; 1 0.8 0.8; 1 0.85 0.85; 1 0.9 0.9];
    M_cols = [0 0 1 ; 0.2 0.2 1; 0.3 0.3 1; 0.4 0.4 1; 0.5 0.5 1; 0.55 0.55 1; 0.6 0.6 1; 0.65 0.65 1; 0.7 0.7 1; 0.75 0.75 1; 0.8 0.8 1; 0.85 0.85 1; 0.9 0.9 1];
    for mm = 1:length(Mx_flights)
            plot3(cortex_flight_struct_resort{Mx_flights(mm)}.pos(1,:),cortex_flight_struct_resort{Mx_flights(mm)}.pos(2,:),cortex_flight_struct_resort{Mx_flights(mm)}.pos(3,:),'Color',M_cols(mm,:)); 
            scatter3(cortex_flight_struct_resort{Mx_flights(mm)}.pos(1,1),cortex_flight_struct_resort{Mx_flights(mm)}.pos(2,1),cortex_flight_struct_resort{Mx_flights(mm)}.pos(3,1),50,'k','filled');
    end
    for mm = 1:length(Kx_flights)
            plot3(cortex_flight_struct_resort{Kx_flights(mm)}.pos(1,:),cortex_flight_struct_resort{Kx_flights(mm)}.pos(2,:),cortex_flight_struct_resort{Kx_flights(mm)}.pos(3,:),'Color',K_cols(mm,:)); 
            scatter3(cortex_flight_struct_resort{Kx_flights(mm)}.pos(1,1),cortex_flight_struct_resort{Kx_flights(mm)}.pos(2,1),cortex_flight_struct_resort{Kx_flights(mm)}.pos(3,1),50,'k','filled');
    end
    hold off;
end
