function mat2ntt(OutputPath, MatlabImpExpCodePath)
if nargin<1
...
if nargin<2
    if strcmp('BATMAN-PC', getenv('computername'))
        ...
        MatlabImpExpCodePath = '/home/madeleine/Desktop/HumanSniffBat/HumanBat/ephys/MatlabImportExport_v6.0.0';
    end
end
...
for Tetrode_i=1:Num_tetrodes % for each of the electrode bundles, eg. tetrodes
    ...
    if ~isempty(Spike_arrival_times) % check if the electrode bundle has no spikes (eg. if all its channels are inactive)
        ...
        NTT_Filename=fullfile(OutputPath,sprintf('%s_%s_TT%s.ntt', BatID, Date,TetrodeID));
