function [Unit_Summary] = HumanBat_UnitSummary(batdate,logger,ciholas_or_cortex,unit,ciholas_flight_struct_resort,B_ephys_data)
...
if ciholas_or_cortex == 'ciholas'

    % SI info and heatmaps
    SI_info = HumanBat_SI_fx(batdate,logger,unit,'all');
    ...
    % Red dot figures
    HumanBat_SI_red_dots(batdate,logger,unit,ciholas_flight_struct_resort,red_dot_clusterable);
    ...
    for i=rdc_split_choice:rdc_split_choice%length(rdc_split)
        ...
        for j=1:5
            [sig_pre(j),sig_land(j)] = HumanBat_MvK_Permutation_Clusters(clus,unit,Flight_Group_Matrix,ciholas_flight_struct_resort,B_ephys_data)
        end
        ...
        if mean(sig_pre) > 0.89
            ...
            t_temp_landing_m = to_list_m'; clear to_list_m;
            to_list_k = [];
            for j=1:length(Kx_flights)
                idx = Kx_flights(j);
                to_list_k = [to_list_k,ciholas_flight_struct_resort{idx}.fend_trimmed/120];
            end
            t_temp_landing_k = to_list_k'; clear to_list_k;
        ...
        if mean(sig_land) > 0.89
            ...
            [PSTH_mn,PSTH_cs,PSTH_up,PSTH_dn] = Raster_AF_v6_ciholas_LAND(s',t_temp_takeoff_m',t_temp_takeoff_k',g_id_m,g_id_k,clr_m,clr_k,interval_,[],[],strcat("ciholasBatUnit ",num2str(unit)," : landing of ciholasFlights To M"),strcat("ciholasBatUnit ",num2str(unit)," : takeoff of ciholasFlights To K"),1,1,Mx_flights,Kx_flights,ciholas_flight_struct_resort,clus);
        else
    ...
    for j=1:length(Mx_flights)
        ...
        plot3(ciholas_flight_struct_resort{i}.pos(:,1), ciholas_flight_struct_resort{i}.pos(:,2),ciholas_flight_struct_resort{i}.pos(:,3),'Color',[0 0 1]);
        
        % Plot human positions
        scatter3(ciholas_flight_struct_resort{i}.pos_K(1:10,1), ciholas_flight_struct_resort{i}.pos_K(1:10,2),ciholas_flight_struct_resort{i}.pos_K(1:10,3),'y','filled');
        scatter3(ciholas_flight_struct_resort{i}.pos_M(1:10,1), ciholas_flight_struct_resort{i}.pos_M(1:10,2),ciholas_flight_struct_resort{i}.pos_M(1:10,3),'cyan','filled');
        scatter3(ciholas_flight_struct_resort{i}.pos_cortex_bat(1,1:10),ciholas_flight_struct_resort{i}.pos_cortex_bat(2,1:10),ciholas_flight_struct_resort{i}.pos_cortex_bat(3,1:10),'green','filled');
    ...
    for j=1:length(Mx_flights)
        ...
        % Plot human positions
        scatter3(ciholas_flight_struct_resort{i}.pos_K(end-10:end,1), ciholas_flight_struct_resort{i}.pos_K(end-10:end,2),ciholas_flight_struct_resort{i}.pos_K(end-10:end,3),'y','filled');
        scatter3(ciholas_flight_struct_resort{i}.pos_M(end-10:end,1), ciholas_flight_struct_resort{i}.pos_M(end-10:end,2),ciholas_flight_struct_resort{i}.pos_M(end-10:end,3),'cyan','filled');
        scatter3(ciholas_flight_struct_resort{i}.pos_cortex_bat(1,end-10:end),ciholas_flight_struct_resort{i}.pos_cortex_bat(2,end-10:end),ciholas_flight_struct_resort{i}.pos_cortex_bat(3,end-10:end),'green','filled');
        scatter3(ciholas_flight_struct_resort{i}.pos(end,1),ciholas_flight_struct_resort{i}.pos(end,2),ciholas_flight_struct_resort{i}.pos(end,3),'black','filled');
        hold off;
    end
    hold off;
