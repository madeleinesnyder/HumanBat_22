function [SI_ABA] = HumanBat_SI_ABA(batdate,logger,unit,flight_cluster_to_smush)
...
% b. Smush all the flights in specified cluster together (take out the resting time) 
for j=1:10
    if flight_cluster_to_smush == j
        index_ = j
        disp(strcat("Concatenating all flights from cluster ",num2str(index_)));
    end
end
clip = 0;
ciholas_flying_r = []; ephys_flying_r = [];
for i=1:size(ciholas_flight_struct_resort,2)
    if ciholas_flight_struct_resort{i}.fclus == index_
        disp(i)
...
figure(); hold on; title(strcat(num2str(flight_cluster_to_smush)," flights")); 
scatter3(ciholas_r(:,1),ciholas_r(:,2),ciholas_r(:,3),0.5,'k');
scatter3(ciholas_flying_r(:,1),ciholas_flying_r(:,2),ciholas_flying_r(:,3)); hold off;
...
save(strcat(exp_data_path,'ephys/logger13/extracted_data/unit',num2str(unit),'_binCount_flight_cluster_',num2str(flight_cluster_to_smush)),'ephys_binCount','coord_pairs','coord_pairs_unique','occMap','binX','binY','-v7.3')
...
%% Now do this all again for the A, B, and A' smushes

% Compute which flights go to M and which go to K
...
% Smush all flights from cluster X to M
clip = 0;
ciholas_flying_r = []; ephys_flying_r = [];
for i=1:size(ciholas_flight_struct_resort,2)
    if ciholas_flight_struct_resort{i}.fclus == index_ & ismember(i,Flight_Group_Matrix.AllFlightsToMadeleine)
        disp(i)
        st = ciholas_flight_struct_resort{i}.fstart_trimmed+clip; ed = ciholas_flight_struct_resort{i}.fend_trimmed+clip;
        ciholas_flying_r = [ciholas_flying_r;ciholas_r(st:ed,:)];
...
% Plot concatenated flights to check
figure(); hold on; title(strcat(num2str(flight_cluster_to_smush)," flights to M")); 
scatter3(ciholas_r(:,1),ciholas_r(:,2),ciholas_r(:,3),0.5,'k');
scatter3(ciholas_flying_r(:,1),ciholas_flying_r(:,2),ciholas_flying_r(:,3)); hold off;
...
% Save figure of the heatmap 
f = figure('Name',strcat("Unit ",num2str(unit)," 2D ratemap, Cluster ",num2str(flight_cluster_to_smush)," Flights to M only")); hold on; colorbar; imagesc(place_map_smoothed_with_NaN'); axis off; title(strcat("Unit ",num2str(unit)," 2D ratemap smooth with NaN")); hold off;
%saveas(f,strcat(exp_data_path,'ephys/logger13/extracted_data/unit_',num2str(unit),'_shuffles/',num2str(batdate),'_14650_PlaceMap',flights_to_smush),'jpg');
SI_info.information_per_spike = information_per_spike;
SI_info.unit = unit;
SI_info.logger = logger;
...
%% Now do this for Kevin

% Smush all flights from cluster X to M
clip = 0;
ciholas_flying_r = []; ephys_flying_r = [];
for i=1:size(ciholas_flight_struct_resort,2)
    if ciholas_flight_struct_resort{i}.fclus == index_ & ismember(i,Flight_Group_Matrix.AllFlightsToKevin)
        disp(i)
        st = ciholas_flight_struct_resort{i}.fstart_trimmed+clip; ed = ciholas_flight_struct_resort{i}.fend_trimmed+clip;
        ciholas_flying_r = [ciholas_flying_r;ciholas_r(st:ed,:)];
...
% Plot concatenated flights to check
figure(); hold on; title(strcat(num2str(flight_cluster_to_smush)," flights to K")); 
scatter3(ciholas_r(:,1),ciholas_r(:,2),ciholas_r(:,3),0.5,'k');
scatter3(ciholas_flying_r(:,1),ciholas_flying_r(:,2),ciholas_flying_r(:,3)); hold off;
...
% Save figure of the heatmap 
f = figure('Name',strcat("Unit ",num2str(unit)," 2D ratemap, Cluster ",num2str(flight_cluster_to_smush)," Flights to K only")); hold on; colorbar; imagesc(place_map_smoothed_with_NaN'); axis off; title(strcat("Unit ",num2str(unit)," 2D ratemap smooth with NaN")); hold off;
%saveas(f,strcat(exp_data_path,'ephys/logger13/extracted_data/unit_',num2str(unit),'_shuffles/',num2str(batdate),'_14650_PlaceMap',flights_to_smush),'jpg');
SI_info.information_per_spike = information_per_spike;
SI_info.unit = unit;
SI_info.logger = logger;
