...
figure(); hold on; scatter3(ciholas_r(bsp1,1),ciholas_r(bsp1,2),ciholas_r(bsp1,3),3,'k','filled');
%scatter3(human_r(bsp1,1,3),human_r(bsp1,2,3),human_r(bsp1,3,3),3,'red','filled');
scatter3(human_r(bsp1,1,4),human_r(bsp1,2,4),human_r(bsp1,3,4),3,'b','filled');
...
f = figure('Name',strcat("Unit ",num2str(unit)," 2D ratemap for ", human_name," position")); hold on; colorbar; imagesc(place_map_smoothed_with_NaN'); axis off; title(strcat("Unit ",num2str(unit)," 2D ratemap smooth with NaN for ",human_name," position. SI: ",num2str(information_per_spike))); hold off;

%% Red Dots figure
figure(); hold on; title(strcat(human_name," flights with bat ephys plotted on top"));
scatter3(human_flying_r(:,1),human_flying_r(:,2),human_flying_r(:,3),2,[0.7 0.7 0.7],'filled');
...
for i=1:length(spike_idxs)
    scatter3(human_flying_r(round(spike_idxs(i)),1),human_flying_r(round(spike_idxs(i)),2),human_flying_r(round(spike_idxs(i)),3),30,'r','filled');
end

%% 5. Check if the shuffles already exist for that cell
% If they exist, run a t test to assess significance of that unit's SI
clear tt temp_rr stretches human_r human_t human_flying_r_ ephys_flying_r_shift ephys_flying_r_ ciholas_ephys_on_cortex ciholas_ephys_on_human ciholas_flight_struct_resort
